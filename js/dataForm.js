  ...
  $(document).on('click','.edit-survey',function(e){
    ...
    $.ajax({
      ...
      success:function(res)
      {
        ...
        $( "option.survey_opt" ).each(function() {
          if($(this).val() == res['survey']['id']){
            $(this).attr('selected','true');
          };
        });
...
$(document).on('click','input#ssm_user, input#ssm_user_edit',function () {
  if (this.checked){
    $('div.for_ship_user').attr('hidden',true);
    $('div.all_user').attr('hidden',false);
    // $('option.admin_role').attr('hidden',false);
...
$(document).on('click','input#srd_user, input#srd_user_edit',function () {
  if (this.checked){
    $('div.for_ship_user').attr('hidden',true);
    $('div.all_user').attr('hidden',false);
    // $('option.admin_role').attr('hidden',false);
...
$(document).on('click','.edit-user',function(e){
  ...
  $.ajax({
    ...
    success:function(res)
    {
      console.log(res[1]['user_type'])
      if( res[1]['user_type']=='ship'){
        ...
        $('#user_edit_form .vessel_not_for_admin').attr('disabled',true);

      }
      else if(res[1]['user_type']=='srd'){
        $('div.for_ship_user').attr('hidden',true);
        $('#srd_user_edit').attr('checked',true);
        $('#ssm_user_edit').attr('checked',false);
        $('#ship_user_edit').attr('checked',false);
        $('#user_edit_form .vessel_not_for_admin').attr('disabled',false);
        $('option.role-srd').attr('hidden',false);
        $('option.role-ssm').attr('hidden',true);
        $('option.ship_role').attr('hidden',true);
      }
      else if(res[1]['user_type']=='ssm'){
        $('div.for_ship_user').attr('hidden',true);
        $('#ssm_user_edit').attr('checked',true);
        $('#srd_user_edit').attr('checked',false);
        $('#ship_user_edit').attr('checked',false);
        $('#user_edit_form .vessel_not_for_admin').attr('disabled',false);
        $('option.role-srd').attr('hidden',true);
        $('option.role-ssm').attr('hidden',false);
...
$(document).on('click','#indSave',function(e){
  ...
  if(role=='am-ssm'){
    var itemId = $(this).parent('td').parent('tr').find('td.deliver_qty input').data('id');
    var itemValue = $(this).parent('td').parent('tr').find('td.deliver_qty input').val();  
        ...
        swal({
          ...
          preConfirm: function() {
            return new Promise(function(resolve) {
              ...
              .done(function(response){
                swal('Congratulation!','Your Requested '+type+ ' restored successfully','success').then(function(){
        ...
        swal({
          ...
          preConfirm: function() {
            return new Promise(function(resolve) {
              ...
              .done(function(response){
                if(response.deleted){
                  swal('Success!','Your Requested '+type+ ' deleted successfully','success').then(function(){
                    table
                    .row("tr#"+tr_id)
                    .remove()
                    .draw();
